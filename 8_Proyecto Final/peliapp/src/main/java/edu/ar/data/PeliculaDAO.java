...
public class PeliculaDAO {
  ...
  "INSERT INTO MOVIES (nombre, descripcion, genero, clasificacion, anio, estrellas, director) VALUES (?,?,?,?,?,?,?)";
  private static final String SQL_UPDATE = 
  "UPDATE MOVIES SET nombre = ?, descripcion = ?, genero = ?, clasificacion = ?, anio = ?, estrellas = ?, director = ? WHERE id = ?";
  ...
  public static List<Pelicula> obtener() {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        pelicula = new Pelicula(id,nombre,descripcion,genero,clasificacion,anio,estrellas,director);
  ...
  public static int insertar(Pelicula peli) {
    ...
    try {
      ...
      ps.setInt(4, peli.getclasificacion());
  ...
  public static Pelicula seleccionarPorId(int id) {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        pelicula = new Pelicula(nombre, descripcion, genero, clasificacion, anio, estrellas, director);
      }
    } catch(SQLException e) {
      e.printStackTrace(System.out);
    } finally {
